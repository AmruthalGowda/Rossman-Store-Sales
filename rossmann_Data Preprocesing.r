...
test = read.csv("C:/Users/gowda/OneDrive/Desktop/IIT COURSES/DATA-ANYLITCS/PROJECT/rossmann-store-sales/original data/test.csv", header = T)
train = read.csv("C:/Users/gowda/OneDrive/Desktop/IIT COURSES/DATA-ANYLITCS/PROJECT/rossmann-store-sales/original data/train.csv", header = T)
store = read.csv("C:/Users/gowda/OneDrive/Desktop/IIT COURSES/DATA-ANYLITCS/PROJECT/rossmann-store-sales/original data/store.csv", header = T)
...
data_function <- function(f){
  dataframe <- fread(f)  #fread of train.csv->sttoring in dataframe object 
  dataframe[, Date:= as.Date(Date)]# every roew value of date convert 
  return(dataframe)
}
# preprocessing
#all entries in- chance row might have diff notatn 
#different format DATES- making similar eg.  d/y/



#new dataframes
train = data_function("C:/Users/gowda/OneDrive/Desktop/IIT COURSES/DATA-ANYLITCS/PROJECT/rossmann-store-sales/train.csv")
test = data_function("C:/Users/gowda/OneDrive/Desktop/IIT COURSES/DATA-ANYLITCS/PROJECT/rossmann-store-sales/test.csv")

#check number of unique values in each column #dp LbyR(library)
...
table(train$Open)
#selected open colum train data- % of zeroes(closed)
...
#histogram for number of sales    #maximum sales ijn that range  # lot of sales in that range 
hist(train$Sales,main="Histogram for sales count in stores",xlab="stores" ,border="black",col="green", breaks=100, ylab = "count of sales")

# lot of sales in that range  
# lot of sales in that range sales are zero
